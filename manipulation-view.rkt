...
(define-type GetHandleFunc (-> (Listof v)))
(define-type UpdateHandleFunc (-> Nonnegative-Integer v Void))
(define-type ButtonPressFunc (-> Nonnegative-Integer Nonnegative-Integer Void)) 

(struct active-drag ([index : Nonnegative-Integer] [rel : v]) #:transparent)
...
(define-provide (handle-view render get-handles update-handle buttons [width 300] [height 300] [handle-radius 6] [sel-box-size 30])
  ...
  (: mouse-pos v)
  (define mouse-pos (v 0 0)) ; the vector of the mouse's last position

  (: hovering? (-> v Boolean))
  (define (hovering? handle-pos)
    (vin-circle? mouse-pos handle-pos handle-radius))

  (: try-drag-handle (-> Nonnegative-Integer v Boolean))
  ...
  (: handle-brush (-> Nonnegative-Integer v (Instance Brush%)))
  ...
  (: draw-handle (-> (Instance DC<%>) Nonnegative-Integer v Void))
  ...
  (define (draw dc)
    ...
    (unless in-view-mode
      ...
      (send dc set-brush regular-brush)
      (map (lambda ([hpair : (Pairof Nonnegative-Integer v)])
  ...
  (define (press x y w h)
    ...
    (if (and (>= x (- w (* sel-box-size (+ 1 (length buttons))))) (<= y sel-box-size))
        ...
        (ormap
         (lambda ([hpair : (Pairof Nonnegative-Integer v)])
